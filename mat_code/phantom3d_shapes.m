function [p,ellipse]=phantom3d_shapes(varargin)
...
[x,y,z] = meshgrid(rng2,rng1,rng3);

coord = [flatten_1(x); flatten_1(y); flatten_1(z)];

p = flatten_1(p);

for k = 1:size(ellipse,1)
   shape = ellipse(k,1) ;
   A = ellipse(k,2);            % Amplitude change for this ellipsoid
   asq = ellipse(k,3);        % a^2
   bsq = ellipse(k,4);        % b^2
   csq = ellipse(k,5);        % c^2
   x0 = ellipse(k,6);           % x offset
   y0 = ellipse(k,7);           % y offset
   z0 = ellipse(k,8);           % z offset
   phi = ellipse(k,9);%*pi/180;   % first Euler angle in radians
   theta = ellipse(k,10);%*pi/180; % second Euler angle in radians
   psi = ellipse(k,11);%*pi/180;  % third Euler angle in radians
   ...
   coordp = alpha*coord;
   if shape == 1
       idx = find((coordp(1,:)-x0).^2./asq^2 + (coordp(2,:)-y0).^2./bsq^2 + (coordp(3,:)-z0).^2./csq^2 <= 1);
   else
       if shape == 2
           idx = find(coordp(1,:)-x0<=abs(asq) & coordp(2,:)-y0<=abs(bsq) & coordp(3,:)-z0<=abs(csq) & coordp(1,:)-x0>=-abs(asq) & coordp(2,:)-y0>=-abs(bsq) & coordp(3,:)-z0>=-abs(csq));
%            idx = alpha*idx;
       end
   end
   p(idx) = p(idx) + A;
...
n = [128 128 128];     % The default size
e = [];
...
for i=1:nargin
   ...
   elseif ndims(varargin{i})==2 && size(varargin{i},2)==11 
